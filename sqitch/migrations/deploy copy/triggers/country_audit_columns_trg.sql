...
CREATE TRIGGER country_audit_columns_trg
    BEFORE UPDATE OR DELETE ON dvdrental.country
    FOR EACH ROW
    EXECUTE FUNCTION dvdrental.audit_columns_trg_func();

COMMENT ON TRIGGER country_audit_columns_trg ON dvdrental.country IS 'Add audit_columns_trg to dvdrental.country.';
