BEGIN;
  ...
  SELECT tables_are('rental', ARRAY['film', 'category', 'film_actor', 'actor', 'country', 'film_category', 'inventory', 'rental', 'store', 'customer', 'language', 'payment', 'address', 'city', 'staff']::TEXT[], 'Schema rental should have the correct tables.');

  SELECT foreign_tables_are('rental', ARRAY[]::TEXT[], 'Schema rental should have the correct foreign tables.');

  SELECT views_are('rental', ARRAY['film_list', 'staff_list', 'nicer_but_slower_film_list', 'actor_info', 'sales_by_film_category', 'customer_list', 'sales_by_store']::TEXT[], 'Schema rental should have the correct views.');

  SELECT materialized_views_are('rental', ARRAY[]::TEXT[], 'Schema rental should have the correct materialized views.');

  SELECT sequences_are('rental', ARRAY['country_country_id_seq', 'inventory_inventory_id_seq', 'rental_rental_id_seq', 'store_store_id_seq', 'language_language_id_seq', 'customer_customer_id_seq', 'address_address_id_seq', 'city_city_id_seq', 'payment_payment_id_seq', 'staff_staff_id_seq', 'film_film_id_seq', 'actor_actor_id_seq', 'category_category_id_seq']::TEXT[], 'Schema rental should have the correct sequences.');
