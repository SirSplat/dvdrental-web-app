# coding: utf-8
from sqlalchemy import ARRAY, Boolean, CHAR, Column, Date, DateTime, Enum, ForeignKey, Index, Integer, Numeric, SmallInteger, String, Table, Text, text
...
class Staff(Base):
    ...
    last_update = Column(DateTime, nullable=False, server_default=text("now()"))
    picture = Column(Text)
    role_type = Column(Text, nullable=False, server_default=text("'trainee'::text"))
...
class Rental(Base):
    ...
    __table_args__ = (
        Index('rental_rental_date_inventory_id_customer_id_uidx', 'rental_date', 'inventory_id', 'customer_id', unique=True),
    )
